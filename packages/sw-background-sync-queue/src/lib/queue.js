...
class Queue {
	...
	async push(request, config){
		try{
			...
			let queuableRequest = await requestManager.getQueueableRequest(request, localConfig);
			_queue.push(hash);
	...
	async cleanupQueue(){
		...
		for(const hash of _queue){
			...
			if (requestData && requestData.metadata.creationTimestamp + requestData.config.maxAge > Date.now() ) {
