...
const uniq = (() => {
  ...
  return (scriptUrl) => {
    ...
    return url.toString();
    // return url.pathname + url.search.slice(0, 4);
  }
})();


const stubAlreadyControllingSw = async () => {
  const scriptUrl = uniq('sw-claim.js');
...
describe(`[workbox-window] Workbox`, function() {
  ...
  xdescribe(`constructor`, function() {
    it(`creates an instance of the Workbox class`, async function() {
      const wb = new Workbox(uniq('sw-empty.js'));
  ...
  xdescribe(`register`, function() {
    it(`registers a service worker if the window is loaded`, async function() {
      ...
      const scriptUrl = uniq('sw-empty.js');
    ...
    describe(`logs in development-only`, function() {
      ...
      it(`(warn) when the registered SW is not in scope for the current page`, async function() {
        ...
        const wb = new Workbox(uniq('../static/sw-empty.js'));
        await wb.register();
      ...
      it(`(warn) when a service worker is installed but now waiting`, async function() {
        ...
        const wb = new Workbox(uniq('sw-no-skip-waiting.js'));
        await wb.register();
      ...
      it(`(error) if the instance has already been unregistered`, async function() {
        const wb = new Workbox(uniq('sw-claim.js'));
        await wb.register();
  ...
  describe(`events`, function() {
    ...
    describe(`installed`, function() {
      it(`fires the first time the registered SW is installed`, async function() {
        const scritUrl = uniq('sw-claim.js');
        ...
        const wb3 = new Workbox(uniq('sw-claim.js'));
    ...
    describe(`externalActivated`, function() {
      it(`runs when an external SW is found and activated`, async function() {
        ...
        await navigator.serviceWorker.register(uniq('sw-skip-waiting.js'));
        await sleep(1000);

        // const wb2 = new Workbox(uniq('sw-skip-waiting.js'));
        ...
        // Assert the same method on the second instance isnt' called.
